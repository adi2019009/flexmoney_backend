	...
	if(user)
	{
	  ...
	  const enrollment = new Enroll({
		...
		try{
			...
			res.status(201).send({enrollment:"YES",fee_payment_date,start_time:slot.startTime,end_time:slot.endTime,prefix:slot.prefix,message:"YOU PAID YOUR FEE"});
		}
		catch(e){
			res.send({enrollment:"NO",message:"UNABLE TO PAY FEE"});
		}
	}
	else 
	{
